...
On these ten tasks both sides' success rates were comparable — what differs is what a run costs and whether it repeats. A fixed `POST /command` sequence needs no model at all, so tokens and waiting time can go to zero; a path where the model decides every step has no such option.
...
- 📦 Releases — [v1.5.1](https://github.com/darrenhooooo/webflow-bridge/releases)
